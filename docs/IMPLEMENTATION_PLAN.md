...
- Phase 3A: implemented in v1.3 with visible controls, active-tab auto-checkpoint, heuristic load indicator, and bootstrap resume without auto-send. v1.5.4 adds direct Project pre-open from the user gesture and an extension-owned background tab controller plus pending handoff persistence in `chrome.storage.local`, a shared-DOM mailbox bridge with visible health status, a user-gesture preopened handoff tab, and bounded recovery if ChatGPT falls back to Home; v1.3.1 also hardened same-Project navigation by preferring ChatGPT's own live Project link over a synthesized route.
