...
- Phase 3B.6: v1.8.2 adds guarded rate-limit auto-dismiss plus smart local backoff. Only known rate-limit dialogs with exact safe acknowledgement labels are eligible; destructive/payment/permission/approval dialogs are excluded. Automatic Retry/Continue, checkpoints, handoff, and raw/handoff exports pause during the 3 → 6 → 10 minute cooling window, which is visible in the Continuity panel.
