...
Use **New Chat Handoff** before a long conversation becomes unreliable. v1.3.4 derives Project scope directly from the live `/c/<conversation>` route, treats the fresh Project composer as the canonical `/project` landing route, and falls back only to that canonical route rather than guessing other Project subpaths. If ChatGPT lands the new tab on Home, one guarded self-heal redirect retries the pending Project landing once. The extension refreshes the checkpoint, creates a bounded bootstrap capsule, opens the same Project in a fresh tab, and attempts to place the bootstrap into the new composer without sending it. If automatic insertion is unavailable, the bootstrap is also copied to the clipboard and **Resume Handoff** remains available.
